...
export default class Editor {
    constructor({ getKey, prompt, customHandlers, handlerFactories, save, close, status, clipboard, selectorsByName, tabSpaces, chars, width, height, screenTop, screenLeft, log, hintStack, screen, languages, language }) {
        this.getKey = getKey;
        this.save = save;
        this.close = close;
        this.status = status;
    ...
    acceptKey(key) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const result = yield this.handleKey(key);
            if (result === false) {
                // Bell would be good here
                return;
            }
            const { selecting, appending, undo } = result || {};
            if (undo) {
    ...
    handleKey(key) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ((handler === null || handler === void 0 ? void 0 : handler.selectionRequired) && !this.selectMode) {
                handler = null;
            }
            if (handler) {
                ...
                for (const handler of this.handlersWithTests) {
                    const result = yield handler.do(key);
                    if (result) {
                        return result;
                    }
                }
    ...
    draw(appending) {
        ...
        if (this.prompt.length) {
            for (let col = 0; (col < this.prompt.length); col++) {
                screen.set(this.screenLeft - this.prompt.length + col, this.screenTop, this.prompt.charAt(col));
            }
        }
        ...
        for (let sy = 0; (sy < this.height); sy++) {
            ...
            for (let sx = 0; (sx < this.width); sx++) {
                ...
                if (selected) {
                    if ((_row > selRow1 || ((_row === selRow1) && (_col >= selCol1))) &&
    ...
    getSelectionChars() {
        const { selRow1, selCol1, selRow2, selCol2, selected } = this.getSelection();
        const result = [];
    ...
    createSubEditor(params) {
        ...
        this.draw();
        const editor = new Editor(Object.assign({ handlerFactories: this.handlerFactories, clipboard: this.clipboard, selectorsByName: this.selectorsByName, tabSpaces: this.tabSpaces, log: this.log, screen: this.screen, languages: this.languages, language: this.languages.default }, params));
        editor.draw();
    ...
    shiftSelection(direction) {
        let { selected, selRow1, selCol1, selRow2, selCol2 } = this.getSelection();
